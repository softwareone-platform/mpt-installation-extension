...
from mpt_installation_extension.routers.events.agreement import process_agreement_complete
from mpt_installation_extension.routers.events.order import process_order_purchase
...
def test_purchase_executes_pipeline(mocker):
    ...
    pipeline = mocker.patch(
        "mpt_installation_extension.routers.events.order.PurchasePipeline", autospec=True
    )
...
def test_agreement_complete_executes_pipeline(mocker):
    ...
    pipeline = mocker.patch(
        "mpt_installation_extension.routers.events.agreement.CompleteAgreementPipeline",
        autospec=True,
    )
